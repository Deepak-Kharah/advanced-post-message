...
describe("ERROR_MESSAGES", () => {
    ...
    it("should have the correct receiveEvent error messages", () => {
        expect(
            ERROR_MESSAGES.receiveEvent.noRequestListenerFound("test")
        ).toEqual('No request listener found for event "test"');
        expect(ERROR_MESSAGES.receiveEvent.codeReturnedError).toEqual(
            "The code returned an error"
        );
        expect(
            ERROR_MESSAGES.receiveEvent.noResponseListenerFound("test")
        ).toEqual('No response listener found for hash "test"');
